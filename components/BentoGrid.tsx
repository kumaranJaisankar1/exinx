...
const BentoBox = ({ title, subtitle, description, features, icon, className, accentColor = 'slate', delay = 0, visual }: BentoBoxProps) => {
    ...
    const accentClasses = {
        brand: 'bg-accent/5 border-accent/20 hover:border-accent/40 ring-1 ring-accent/10',
        gold: 'bg-amber-950/10 border-amber-500/20 hover:border-amber-400 shadow-none',
        slate: 'bg-slate-900/60 border-slate-800 hover:border-slate-600 shadow-none'
    };
    ...
    return (
        ...
        >
            {/* Module ID Tag */}
            <div className="absolute top-0 right-12 px-4 py-1.5 bg-white/5 border-x border-b border-white/10 rounded-b-xl text-[9px] font-mono text-slate-500 tracking-widest z-20">
            ...
            <div className="relative z-10 h-full flex flex-col md:flex-row gap-8 lg:gap-12">
                <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-6 lg:mb-8">
                        <div className={cn("p-3 lg:p-4 rounded-2xl bg-white/5 border border-white/10 group-hover:scale-110 transition-transform duration-500", titleClasses[accentColor as keyof typeof titleClasses])}>
                            {icon}
                        </div>
                        <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest px-3 py-1 bg-white/5 rounded-full border border-white/5">
                    ...
                    <div className="space-y-4 lg:space-y-6">
                        <div className="grid grid-cols-1 gap-2 lg:gap-3">
                            {features.map((feature, i) => (
                                <div key={i} className="flex items-center gap-3 text-xs text-slate-300 group-hover:text-white transition-colors">
                                    <div className={cn("w-1.5 h-1.5 rounded-full", i === 0 ? (accentColor === 'brand' ? "bg-accent" : accentColor === 'gold' ? "bg-amber-500" : "bg-slate-300") : "bg-slate-700")} />
                                    {feature}
                                </div>
                            ))}
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-4 border-t border-white/5 pt-6">
                            <div>
            ...
            <div className={cn(
                "absolute -bottom-20 -right-20 w-64 h-64 blur-[100px] opacity-20 transition-opacity duration-700 group-hover:opacity-40",
        ...
        <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="relative w-full h-full max-w-[400px] max-h-[400px]">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    />
                    <div className="relative z-10 w-40 h-40 border border-accent/10 rounded-full flex items-center justify-center backdrop-blur-md bg-white/5">
                        ...
                        <motion.div
                            ...
                            className="absolute inset-2 border-t border-l border-accent/20 rounded-full"
                        />
                    </div>
                </div>
                ...
                {[1, 2, 3, 4].map((i) => (
                    <motion.div
                        ...
                        className="absolute inset-0 border border-accent/5 rounded-full"
                ...
                <div className="absolute inset-0 pointer-events-none p-4">
                    <div className="absolute top-[15%] left-[5%] bg-accent/10 border border-accent/20 rounded-md p-1.5 backdrop-blur-xl">
                        <div className="text-[7px] font-mono text-accent uppercase">Input_Stream</div>
                        <div className="text-[9px] font-mono text-white">4.8 GB/s</div>
                    </div>

                    <div className="absolute bottom-[25%] right-[5%] bg-accent/10 border border-accent/20 rounded-md p-1.5 backdrop-blur-xl text-right">
                        <div className="text-[7px] font-mono text-accent uppercase">Logic_Auth</div>
                        <div className="text-[9px] font-mono text-white text-accent">VERIFIED</div>
                    </div>
                ...
                {/* System Logs */}
                <div className="absolute bottom-6 left-6 font-mono text-[7px] text-accent/30 leading-loose">
        ...
        <section className="py-32 px-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {[1, 2, 3, 4].map((i) => (
                                    <motion.div
                                        ...
                                        className="w-8 h-8 rounded-lg bg-slate-500/20 border border-slate-500/20"
                                    />
                                ))}
                            </div>
                        </div>
                            ...
                            <ClientOnly>
                                <div className="relative w-full h-full flex items-center justify-center">
                                    <motion.div
                                        ...
                                        className="w-32 h-32 border border-amber-500/20 rounded-full flex items-center justify-center"
                                    >
                ...
                <motion.div
                    ...
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    className="lg:col-span-12 mt-12 bg-white/[0.02] border border-white/5 rounded-[40px] p-8 md:p-16 flex flex-col md:flex-row items-center justify-between gap-12 backdrop-blur-3xl overflow-hidden relative"
                >
                    <div className="space-y-4 relative z-10 text-center md:text-left">
                        <h4 className="text-3xl md:text-4xl font-display font-bold text-white tracking-tight">Ready to integrate <br className="hidden md:block" /> the future?</h4>
                        <p className="text-slate-400 max-w-md font-light text-sm md:text-base">Join the private network of institutions already operating on the EXINX architecture.</p>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-4 md:gap-6 relative z-10 w-full sm:w-auto">
                        <button className="px-8 md:px-12 py-4 md:py-5 bg-accent text-white font-bold rounded-full hover:bg-accent/80 transition-all hover:scale-105 shadow-[0_0_40px_rgba(0,213,190,0.2)] uppercase text-[10px] md:text-xs tracking-widest whitespace-nowrap">
                            Request Deployment
                        </button>
                        <button className="px-8 md:px-12 py-4 md:py-5 border border-white/10 text-white font-bold rounded-full hover:bg-white/5 transition-all text-[10px] md:text-xs tracking-widest uppercase whitespace-nowrap">
                            Technical Audit
                        </button>
                    </div>
