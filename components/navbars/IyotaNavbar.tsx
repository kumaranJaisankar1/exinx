    ...
    <nav className="absolute top-0 left-0 right-0 z-[100] py-6 px-6 md:py-10 md:px-10 bg-transparent">
      <div className="max-w-[1400px] mx-auto flex justify-between items-center">
        ...
        <div className="md:hidden flex items-center gap-4">
          ...
          >
            <span className={cn("w-6 h-0.5 transition-all bg-foreground", isMenuOpen && "rotate-45 translate-y-2")} />
            <span className={cn("w-6 h-0.5 transition-all bg-foreground", isMenuOpen && "opacity-0")} />
            <span className={cn("w-6 h-0.5 transition-all bg-foreground", isMenuOpen && "-rotate-45 -translate-y-2")} />
          </button>
        </div>
      </div>
          ...
          <>
            ...
            >
              <ul className="flex flex-col gap-8 text-right list-none px-4">
                    ...
                    <li key={link.label}>
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive ? "text-[#FF0000]" : "text-muted-foreground hover:text-foreground"
                        )}
                        onClick={() => setIsMenuOpen(false)}
                      >
                        {link.label}
                ...
                <li className="pt-4">
                  <button
                    className="w-full px-6 py-4 bg-[#FF0000] text-white font-bold text-xs rounded-full uppercase tracking-widest whitespace-nowrap"
                    onClick={() => {
                      setIsMenuOpen(false);
                      if (typeof window !== 'undefined') {
